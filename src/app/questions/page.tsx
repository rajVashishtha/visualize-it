  ...
  return (
    <main className="min-h-screen bg-orange-100 px-4 py-10">
      <div className="flex flex-col justify-center items-center">
        <h1 className="text-3xl font-bold mb-10 text-gray-800">Choose an Algorithm</h1>
          ...
          <table className="min-w-full bg-white rounded-xl shadow-md">
            ...
            <tbody>
              {questions.map((question) => (
                <tr key={question.name} className="border-t border-orange-300 hover:bg-orange-50 transition">
                  <td className="py-3 px-4 text-gray-800">{question.name}</td>
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded-full text-sm font-medium ${getDifficultyClass(question.difficulty)}`}>
                      {question.difficulty}
                    </span>
                  </td>
                  <td className="py-3 px-4">
